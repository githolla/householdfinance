...
- Colours live in the `C` object and the CSS variables. The theme is light, modern, and muted (Linear/Mercury register, nothing saturated): near-white ground `#F6F6F5`, white cards with hairline borders and a whisper of shadow, ink `#18181B`. Primary buttons are ink-filled; `--acc` `#5B5BD6` (muted indigo) is reserved for focus rings, links, and small accents; active nav is a neutral gray pill. Dusty indigo `#7086D6` is partner A, dusty teal `#55A297` partner B, bronze `#C0965C` shared/goals, muted red `#D4574E` the only alarm. `GROUP_COLORS` gives each envelope group its own muted hue for the donut and chips.
- Type: Inter (everything), Space Grotesk (headings and display numbers keep `.serif`/h1–h3), figures tabular via `font-variant-numeric`. Loaded via `@import` in the CSS string. No uppercase-letterspaced display type; tiny uppercase is reserved for KPI labels and nav section headers.
- The shell is a white left sidebar on desktop (grouped nav with inline SVG icons from `IC`, an unpaid-bills badge, household avatar, and the compact Daily bread card at the bottom) and a fixed bottom icon bar on mobile (≤900px). The Overview opens with the `Concierge` bar, then the available-to-spend hero card that also carries the thesis sentence and the allocation rail. Setup keeps the light gradient `.hero` band.
