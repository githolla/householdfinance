...
- The shell is a dark left sidebar on desktop (grouped nav with inline SVG icons from `IC`, an unpaid-bills badge, household avatar, balance + build tag + start-clean + Daily bread in the footer) and a fixed bottom icon bar on mobile (≤900px). The Overview is **at-a-glance**: a 4-KPI row (current balance [dark] · income · spent · available), a compact `Concierge` bar, the `HealthCard`, then cards — a spending **donut** (`Where it went`), **recent transactions**, **pots** progress, **recurring-bill status** tiles, **coming up** (plan-ahead scenarios), a compact **giving** card, and the six-month cash-flow chart + planner notes. Setup keeps a paper-gradient `.hero` band.
