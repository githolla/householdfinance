...
- Colours live in the `C` object and the CSS variables. The theme is light and modern: near-white ground `#F7F6F3`, white cards with soft shadows, near-black text. Blue `#2563EB` (`--acc`) is the interactive accent (primary buttons, active nav, links); amber `#F59E0B` stays the shared/goals colour; blue `#3B82F6` is partner A, teal `#14B8A6` partner B, red `#EF4444` the only alarm colour. `GROUP_COLORS` gives each envelope group its own hue for the donut and chips.
