...
- **Daily study**: a short daily Bible study on money and wisdom lives in `src/scripture.js` (`STUDIES` + `studyForDay`, deterministic by calendar day) — each has a passage (WEB), a two-line reflection, a "talk about it" question for the couple, and a one-line prayer. The Overview leads with a green **Today's study** banner (`.studybanner`) and the sidebar Daily bread card both open the `DailyStudy` modal (opened via `ctx.openStudy`, gated on `m.faithOn`). Add studies to `STUDIES`; keep them WEB and finance/wisdom themed.
